...
class GraspSingleInSceneEnv(CustomSceneEnv):
    ...
    def _initialize_actors(self):
        ...
        if obj_init_xy is None:
            obj_init_xy = self._episode_rng.uniform([-0.35, 0.0], [-0.1, 0.42], [2])
        ...
        if len(self.distractor_objs) > 0:
            ...
            object_positions = [obj_init_xy]  # start with main object position
            ...
            for distractor_obj in self.distractor_objs:
                ...
                if distractor_init_xy is None:
                    while True:
                        distractor_init_xy = self._episode_rng.uniform([-0.3, 0.05], [-0.1, 0.42], [2])
                        # Check distance from all other objects
                        valid_position = True
                        for i, pos in enumerate(object_positions):
                            if i == 0 or i == 1:
                                if np.linalg.norm(distractor_init_xy - pos) < 0.2:
                                    valid_position = False
                                    break
                            elif np.linalg.norm(distractor_init_xy - pos) < 0.1:
                                valid_position = False
                                break
                        if valid_position:
                            break
                    # else:
                    #     # If we couldn't find a valid position after 100 tries, use the last attempted position
                    #     pass
                    
                    object_positions.append(distractor_init_xy)

                p = np.hstack([distractor_init_xy, obj_init_z])
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.orientations_dict = {
            ...
            "custom": euler2quat(*custom_orientation) if custom_orientation else None,
        }
        super().__init__(**kwargs)
...
@register_env("WTScene-v0", max_episode_steps=80)
class WTScene(GraspSingleCustomOrientationInSceneEnv):
    def __init__(self,rgb_overlay_filename="real_inpainting/google_coke_can_real_eval_1.png", **kwargs):
        self.rgb_overlay_filename = rgb_overlay_filename
        kwargs.pop("model_ids", None)
        self.distractor_model_ids = kwargs.pop("distractor_model_ids", None)
        kwargs["model_ids"] = self.distractor_model_ids[0]
        self.distractor_model_ids = self.distractor_model_ids[1:]
        kwargs["distractor_model_ids"] = self.distractor_model_ids

        self.region = kwargs.pop("region", [[-0.3, 0.05], [-0.05, 0.42]])
        
        custom_orientation = kwargs.pop("custom_orientation", None)
        super().__init__(custom_orientation=custom_orientation, **kwargs)
    ...
    def _setup_prepackaged_env_init_config(self):
        ...
        ret["rgb_overlay_path"] = str(ASSET_DIR / self.rgb_overlay_filename)
        return ret
    ...
    def _initialize_actors(self):
        ...
        obj_init_xy = self.obj_init_options.get("init_xy", None)
        if obj_init_xy is None:
            obj_init_xy = self._episode_rng.uniform(self.region[0], self.region[1], [2])
        obj_init_z = self.obj_init_options.get("init_z", self.scene_table_height)
        obj_init_z = obj_init_z + 0.5  # let object fall onto the table
        obj_init_rot_quat = self.obj_init_options.get("init_rot_quat", [1, 0, 0, 0])
        p = np.hstack([obj_init_xy, obj_init_z])
        ...
        if len(self.distractor_objs) > 0:
            done_select = False
            count = 0
            while not done_select:
                count += 1
                candidates = []
                for i in range(len(self.distractor_objs)+1):
                    candidates.append(self._episode_rng.uniform(self.region[0], self.region[1], [2]))
                points = np.array(candidates)
                diff = points[:, np.newaxis, :] - points[np.newaxis, :, :]
                pairwise_distances = np.linalg.norm(diff, axis=-1)
                np.fill_diagonal(pairwise_distances, np.inf)
                if np.all(pairwise_distances >= 0.18):
                    done_select = True
                if count > 100000:
                    break
            p = np.hstack([candidates.pop(0), obj_init_z])
            self.obj.set_pose(sapien.Pose(p, q))
            ...
            for i, distractor_obj in enumerate(self.distractor_objs):
                ...
                distractor_init_xy = distractor_obj_init_options.get("init_xy", None)
                if distractor_init_xy is None:
                    distractor_init_xy = candidates[i]
